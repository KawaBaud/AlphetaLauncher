...
import io.github.KawaBaud.launcher.minecraft.MinecraftUpdateWorker;
import io.github.KawaBaud.launcher.util.LauncherLanguageUtils;
import io.github.KawaBaud.launcher.util.LauncherUtils;
import io.github.KawaBaud.launcher.util.MinecraftUtils;
...
public class MinecraftAppletWrapper extends JApplet implements AppletStub {
	...
	@Getter
	@Setter
	private transient ClassLoader mcAppletClassLoader;
	@Getter
	@Setter
	...
	@Getter
	@Setter
	private boolean updaterTaskStarted;
	@Getter
	@Setter
	private boolean updaterTaskErrored;
	...
	public MinecraftAppletWrapper(String username, String sessionId, boolean demo) {
		...
		if (IntStream.rangeClosed(3, 5).anyMatch(i -> selectedVersion.contains(String.format("1.%d.", i)))) {
	...
	public void paint(Graphics g) {
		...
		if (this.bImg == null
			...
			this.bImg = configuration.createCompatibleImage(appletWidth >> 1, appletHeight >> 1, Transparency.OPAQUE);
		}
		...
		try {
			...
			String title = updaterTaskErrored ? bundle.getString(LauncherLanguageUtils.getGAWKeys()[1])
